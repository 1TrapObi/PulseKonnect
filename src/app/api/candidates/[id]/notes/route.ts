...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const noteType = body.type ? String(body.type) : "general";
    ...
    const { data: note, error } = await admin
      ...
      .select("id,candidate_id,user_id,note_type,content,is_internal,created_at,updated_at")
      .maybeSingle();
    ...
    await admin.from("activities").insert([
      {
        ...
        notes: JSON.stringify({ note_type: noteType, is_internal: isInternal }),
      },
    ]);
